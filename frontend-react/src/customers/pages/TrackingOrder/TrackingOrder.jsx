...
//   console.log("Auth State:", auth.jwt);
//   const {
//     currentOrder: order,
//     loading,
//     error,
...
//   console.log("Order State:", order, loading, error);
...
//       if (
//         !auto &&
//         (!ui.canConfirm || getActiveStep(processedOrder?.status) !== 4)
//       )
//         return;
...
//       }
//     },
//     [ui.canConfirm, processedOrder?.status]
//   );
...
//   // const active = getActiveStep(processedOrder.status);
//   const active = getActiveStep(currentStatus);
...
//   return (
//     <div className="p-5 sm:p-8 max-w-7xl mx-auto min-h-screen bg-white font-sans">
//       <style>{style}</style>
//       <h1 className="text-3xl sm:text-4xl font-extrabold text-[#00b14f] mb-8">
...
//         {/* <StatusStepper orderStatus={processedOrder.status} /> */}
//         <StatusStepper orderStatus={currentStatus} />
...
//               onDelivered={async () => {
//                 // Gọi API update status → DELIVERED
//                 await dispatch(
//                   updateOrderStatus({
...
//             <ConfirmationAlert
//               // ui={{ ...ui, orderStatus: processedOrder.status }}
//               ui={{ ...ui, orderStatus: currentStatus }}
//               confirm={confirm}
//             />
//           </>
//         )}
...
// src/customers/pages/TrackingOrder/TrackingOrder.jsx
...
export default function TrackingOrder() {
  ...
  const confirm = useCallback(
    async (auto = false) => {
      if (!auto && (!ui.canConfirm || getActiveStep(currentStatus ?? order?.status) !== 4))
        return;
      setUi((prev) => ({ ...prev, confirming: true, confirmError: "" }));
      try {
        await new Promise((r) => setTimeout(r, 1000));
    ...
    <div className="p-5 sm:p-8 max-w-7xl mx-auto min-h-screen bg-white font-sans">
      ...
      <div className={`bg-white p-6 rounded-xl border transition-all ${card}`}>
        ...
        <div className="mt-4 p-4 bg-green-50 border-l-4 border-[#00b14f] rounded-r-md">
          <p className="flex items-center text-sm text-gray-700">
            Địa chỉ:{" "}
            <span className="font-bold ml-1">{processedOrder.address.street}</span>
          </p>
          <p className="flex items-center text-sm text-gray-700 mt-1">
            Drone: <span className="font-bold ml-1">{processedOrder.droneId}</span>
          </p>
        </div>
        ...
        {active >= 4 && (
          <>
            ...
            <ConfirmationAlert
              ui={{ ...ui, orderStatus: currentStatus ?? processedOrder.status }}
              confirm={confirm}
            />
          </>
        )}
